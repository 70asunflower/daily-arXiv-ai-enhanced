...
> **TL;DR:** 联邦可解释AI（FedXAI）综述，非推理/通信/容错系统论文。

**研究问题：** 纯综述，未聚焦具体系统/硬件层面gap。
**硬件与互联：** 论文摘要未明确
**方法：** 该文为综述论文，不涉及具体硬件平台、系统实现或量化实验；无硬件平台、无关键技术、无实现层级。
**并行 / 通信 / 调度 / 容错：** 论文摘要未明确
**内存与 KV cache：** 论文摘要未明确
**关键结果：** 无量化结果。综述仅列举了通信效率、非IID数据可解释性等挑战方向，未给出任何数字指标。
**对比基线：** 论文摘要未明确
**测量口径：** 无实验配置、无负载、无指标定义、无数字。
**对应 A/B/C：** Background
**对 7×Thor 启发：** 关联有限。该综述讨论联邦学习中的可解释性，不涉及大模型推理的通信/并行/调度/容错/内存等系统问题，对7×Jetson Thor无直接可迁移技术。
**基础设施假设：** 论文摘要未明确
**无 NVLink 是否成立：** 不适用。该论文不依赖具体互联假设，但与受限以太网下的LLM推理场景无关。
**差异化研究点：** 无。完全不属于A/B/C三条研究路径中任何一条。
**建议精读：** — &nbsp;|&nbsp; **开源：** 未公开

<details>
  <summary>Details</summary>

**Motivation:** 聚焦联邦学习与可解释AI交叉领域，而非大模型分布式推理的通信/调度/容错问题。

**Result:** 论文为综述，未提供任何具体量化结果（吞吐、延迟、带宽、加速比等）。

**Conclusion:** FedXAI需解决通信效率、非IID数据下的可解释性及标准化基准等关键挑战。
...
> **TL;DR:** Agora系统在互联网级异构GPU集群上实现高效大模型预训练，效率达集中式H100的63%。

**研究问题：** 互联网级分布式训练中，节点间带宽极低（~50-100 Mbps）、节点异构且可随时加入/离开，现有DP/MP方法依赖高带宽同构互联，无法直接适用。
**硬件与互联：** 330个消费级GPU节点（以RTX 3090/4090等消费者GPU为主），互联为普通互联网（未标明具体带宽，但远低于数据中心级），无NVLink。
**方法：** Agora使用带宽高效的流水线并行（PP）切分，每个节点仅持有模型一个stage；结合多参与方容错集合通信（fault-tolerant collective ops）。硬件平台为消费级GPU（如RTX 3090/4090等），互联为普通互联网线路。实现层级为系统/分布式训练框架。
**并行 / 通信 / 调度 / 容错：** 带宽高效的流水线并行（Pipeline Parallelism, PP）模型分片，互联网级链接；多参与方容错集合通信（multi-party fault-tolerant collective ops）；异步优化。
**内存与 KV cache：** 论文摘要未明确
**关键结果：** - 模型规模：8.6B参数（Pluralis-8B）\n- 数据量：500B tokens FineWeb-Edu\n- 训练时间：40天\n- 参与节点：330个（贡献节点，消费级GPU，互联网连接）\n- 持续吞吐：~170k tokens/s\n- 计算效率：4.2 tokens per TFLOP\n- 相对效率：集中式H100基线的63%\n- 收敛质量：接近集中式参考运行（within a small margin）
**对比基线：** vs 集中式H100集群（单一数据中心，高带宽NVLink互联），测量口径为计算效率（tokens per TFLOP）和模型收敛损失。
**测量口径：** 实验配置：330节点互联网级分布式预训练；负载：FineWeb-Edu数据集共500B tokens；指标：持续吞吐（~170k tokens/s）、每TFLOP产出tokens数（4.2 tokens/TFLOP）、相对效率（集中式H100基线63%）、收敛质量（与集中式参考运行对比）。论文未明确重复次数与置信区间。
**对应 A/B/C：** Background
**对 7×Thor 启发：** 直接启发：Agora的带宽高效PP分片设计（每个节点只负责一个stage）和容错集合通信机制，可复用到7×Jetson Thor的分布式推理场景——尤其是节点掉队/断开时的容错处理。但注意本文面向训练而非推理，流水线切分策略需要适配推理的prefill/decode阶段。
**基础设施假设：** 依赖互联网级链接（~50-100 Mbps），极低带宽；节点可随时加入/离开；无NVLink。
**无 NVLink 是否成立：** 完全成立。本文场景正是无NVLink、极低带宽（互联网）的异构集群，其容错集合通信和带宽高效PP设计正好适配7×Thor的无NVLink、25GbE环境。
**差异化研究点：** Agora面向训练，可将其PP分片和容错通信思路迁移到推理场景：1）无NVLink下PP+TP混合并行的容错调度策略；2）互联网级节点动态加入/离开对推理延迟（TTFT/TPOT）的影响度量；3）低带宽下KV cache跨节点迁移与Agora的流水线分片结合。
**建议精读：** — &nbsp;|&nbsp; **开源：** 未公开

<details>
  <summary>Details</summary>

**Motivation:** 现有大模型训练依赖数据中心的高带宽同构GPU和单一调度器，而大量消费级GPU（异构、可抢占、互联网连接）无法有效利用。论文要解决如何在互联网级、无高速互联的异构集群上进行高效分布式训练。

**Result:** 完成Pluralis-8B（8.6B参数模型）在500B tokens FineWeb-Edu上的开放预训练，耗时40天，330个贡献节点（以消费级GPU为主，互联网连接）。持续吞吐~170k tokens/s，计算效率4.2 tokens per TFLOP，达到集中式H100基线效率的63%，收敛质量接近集中式参考运行。

**Conclusion:** 在互联网级、异构、可抢占的消费级GPU集群上，通过带宽高效的PP切分和容错集合通信，可以完成数十亿参数模型的预训练，效率可达数据中心级的60%以上。
...
> **TL;DR:** 针对 vDiT 视频扩散 Transformer 的算法-硬件协同设计，利用通道时空相关性实现 5.9×加速。

**研究问题：** 视频扩散 Transformer 自注意力计算在扩散步数减少后成为主要瓶颈，现有稀疏注意力方法未利用视频时空相关性。
**硬件与互联：** 论文摘要未明确（描述为 systolic array like accelerator 定制 ASIC，非现成商用 GPU；节点数/互联/内存配置未提及）
**方法：** 硬件平台：自定义 systolic array 加速器（非 Jetson Thor / A100 / H100 / FPGA / ASIC 等通用平台）。关键技术：轻量级 channel-wise 重用算法，跳过冗余计算并重用部分结果；可重构 PE + 轻量级数据调度器缓解不规则稀疏与数据访问模式。实现层级：算法-硬件协同（硬件架构级）。
**并行 / 通信 / 调度 / 容错：** 论文摘要未明确（本文侧重单加速器内的计算重用与数据调度，不涉及多节点通信/并行/调度/容错）
**内存与 KV cache：** 论文摘要未明确（本文针对 vDiT 中的自注意力计算加速，不涉及 LLM 的 KV cache 机制）
**关键结果：** 1. 相比于 SOTA 加速器，Kaleido 实现 up to 5.9× 速度提升；2. 能效提升 16.0×；3. 生成质量优于先前方法（>17 dB）。具体对比基线未在摘要中注明模型名称。
**对比基线：** 对比对象为"state of the art accelerators"，未指明具体加速器名称或配置。
**测量口径：** 三种主流 vDiT 模型上评估；指标为加速比（speedup）、能效比（energy savings）、生成质量（dB）；重复次数与置信区间未在摘要中说明。
**对应 A/B/C：** Background
**对 7×Thor 启发：** 关联有限。本文是定制加速器设计，无 NVLink 25GbE 边缘集群的分布式推理问题完全不同，通道复用思路不适用于通用 GPU 上的大模型推理。
**基础设施假设：** 论文摘要未明确（推测为数据中心级定制硬件，无多节点互联假设）
**无 NVLink 是否成立：** 不直接相关。本文不依赖 NVLink 或以太网，而是单芯片定制硬件设计，无法直接迁移到无 NVLink 的受限以太网集群环境。
**差异化研究点：** 无直接差异化空间。本文是定制硬件工作，与 7×Jetson Thor 在受限以太网上的分布式 LLM 推理没有交集。
**建议精读：** — &nbsp;|&nbsp; **开源：** 未公开

<details>
  <summary>Details</summary>

**Motivation:** vDiT 视频扩散模型计算开销极高（长扩散步数 + 自注意力），现有稀疏注意力方法忽略了视频数据的独特时空相关性。

**Result:** 在三种主流 vDiT 模型上，Kaleido 相比 SOTA 加速器达 5.9× 加速比和 16.0× 能效提升；生成质量优于先前方法（>17 dB）。

**Conclusion:** 利用视频时空相关性的通道复用可以高效加速 vDiT，但结论依赖定制硬件，不直接适用于通用 GPU 推理。
...
### [15] [MARS: Multi-stage Accelerated Read Stack for Large-buffer Buffered Reads](https://arxiv.org/abs/2607.13604)
